...
def Insert_RDS(text_data, ITEM_KEY, bucket):
    ...
    try:
    	...
    	code, msg = e.args
    	print("ERROR: Insert Fail. Code:",code," message:", msg)
    	sys.exit(2)
    ...
    '''
    ### CREATE TABLE
    sql_create_option = 'create table if not exists Boxes (Item_id int PRIMARY KEY, Coord varchar(1000));'
    print('init table :',sql_create_option )
    with conn.cursor() as cur:
        cur.execute(sql_create_option)
        conn.commit()
    print('init table successed')
    ...
    data_keys = 'Item_id, Coord'
    
    data_vals = str(item_dict['Item_id'])+', '+ '\''+str(text_data.box)+'\''
    print('insert into Boxes ('+data_keys+') values('+data_vals+')')

    
    try:
        with conn.cursor() as cur:
            cur.execute('insert into Boxes ('+data_keys+') values('+data_vals+')')
            conn.commit()
        print("Added items from RDS MySQL table")
        print('CodePipeline TEST')
